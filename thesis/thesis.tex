...
def s(event):
  ...
  ~
	\begin{minipage}{0.45\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
			\node[state,initial] (q0) {$q_0$};
			\node[state,accepting] (qy) [right of=q0] {$q_y$};
			\node[state] (f) [below of=qy] {$f$};
			\node[state,accepting] (qw) [below of=q0] {$q_w$};
...
\begin{figure}[h!]
	\begin{minipage}{0.9\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
        ...
        (q0) edge node {$\bar{E} \vee \bar{E'}$} (fail);
		\end{tikzpicture}
	\end{minipage}
  \caption{A simplified version of the automata from
    Figure~\ref{figure-assert-composition-example}.}
