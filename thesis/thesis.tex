...
\begin{itemize}
	...
	\item In \textit{pythonrv} a \textit{specification function} is a function
		describing a specification, which \textit{pythonrv} can
		use for verification of the program.

	\item A specification function
		\textit{monitors} points \rephrase of the programs (functions), and the
		points being monitored are called \textit{monitorees} \rephrase.
\end{itemize}
...
\textit{unittest} framework that is included in all modern versions. Not much
development has happened on it in the last years. Many new frameworks have have
spawned, such as PyUnit, Nose and py.test. They build upon the style of
unittest,and mostly add new miscellaneous features, such as better test
reporting. The original structure of the unit tests is still prevalent.
unittest builds on the xUnit style, discussed in
...
imports the \texttt{rv} module from the \textit{pythonrv} package. On line 2 it
imports the module containing the function we wish to monitor. Line 5 defines
...
decorators.} \texttt{rv.monitor}. \texttt{rv.monitor} declares that the
function \texttt{fib} in \texttt{fibmodule} should be monitored, and, whenever
...
monitored functions are called. Which function was called can be determined
from the \texttt{event} argument, as is done on lines 6 and 9. It is the
\texttt{called} attribute of a function in the \texttt{event.fn} structure that
allows for this.
...
won't be used in verification any longer.}
...
subsequent events. This is done on line 22.
...
function that takes in a function and returns a function, possibly modifies it,
or uses it in some way (decorates it) in the processs. This is used throughout
python to turn functions into static or class methods, for instance.
...
\footnotetext[4]{This is not true of closures - function defined inside other
...
are executable, and executing them on the correct events, providing access to
the appropriate data, verifies that the specification they represent is
followed.

Specification functions notifies verification violations, that the
specifications are not followed, by raising exceptions of the type
\texttt{AssertionError}. These exceptions are raised when the \texttt{assert}
statement fails. They can also be raised manually: \texttt{raise
...
explicitly remove themselves by calling one \texttt{event.finish},
\texttt{event.success} or \texttt{event.failure} (described in
Section~\ref{section-approach-syntax}).

\subsection{Dealing with Errors}

Whenever a specification violation occurs, and a \texttt{AssertionError} is
raised, it is passed to a \textit{error handler}. There are two built-in error
handlers: One, the default, that re-raises the exception, and thus crashes the
program (unless some other part of the code suppresses the exception), and a
second which just logs the error message to a file.
...
verification could be used to mitigate this.
...
\todo{Results (un)expected? Larger context.}
...
\todo{What do we see in the future? How can this be extended, continued?}

\section{Discussion}

The trend of software systems in general seems to be toward
larges and more complex entities \rephrase. This makes the automated
verification of program correctness, formal or not, every more important and an
essential part of software development. Runtime verification could have a place
there, if it becomes more popular, and simpler to integrate and use in ordinary
software.

The implementation described in this report, \textit{pythonrv}, is freely
available on the web\footnote{\texttt{https://github.com/tgwizard/pythonrv}} as
free, open source software. People are free to try it, incorporate it into
