...
%%%%%%%%%%%%%%%%%%%%%%%%
%% Automata for the four basic formal specification functions
%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h!]
	\begin{minipage}{0.45\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
		...
		\end{tikzpicture}
    \subcaption{The \textit{null} specification.}
	\end{minipage}
  ~
	\begin{minipage}{0.45\textwidth}
		\centering
  ...
  ~
	...
	\caption{The automata for the four basic formal specification functions.}
...
Due to the identity-function nature of the \textit{null} specification, it will
be left out of the composition rules below.


\subsubsection{Composition of the \textit{assert} specification}

Composing with an \textit{assert} specification $a$ is simple when $a$ is on
the left-hand side of the composition. See
Figures~\ref{figure-assert-composition-assert},~\ref{figure-assert-composition-next}
and~\ref{figure-assert-composition-if}.

The automata corresponding to the specifications created by the compositions
from are quite simple. These are created by just taking the expression $E$ from
$a$ and adding it (as part of a conjunctive) to any expressions on all
transitions from the start state in the right-hand side automaton.
...
def s(event):
  ...
  \caption{A composition of an \textit{assert} specification and an \textit{if}
    ...
    would draw another transition from $q0$ to the automaton inserted at that
    composition point labeled $E \wedge \bar{E'}$. $q1$ is the state
    representing the "fallthrough" of the if statement, at the \textit{after}
    composition point}
...
called \textit{next}, which is required, and one called \textit{after}, which
is optional. Compositions using the \textit{after} composition point are
reflexive: $n \, \circ_{after} \, s = s \, \circ_{after} \, n$. For composition
rules concerning $a \, \circ_{after} \, n$ and $i \, \circ_{after} \, n$, see
the sections for \textit{assert} and \textit{if} specifications. Also note
that:

\[
(n \, \circ_{next} \, s) \, \circ_{after} \, (n' \, \circ_{next} \, s')
=
n \, \circ_{next} (s \, \circ_{after} \, s')
\]

As described in section~\ref{section-approach-python-subset}, composition with
a \textit{next} specification on the left hand side is simple. It is
illustrated again in Figure~\ref{figure-next-composition-s}.
...
def s(event):
  ...
  ~
	\begin{minipage}{0.4\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
			\node[state,initial] (q0)	{$q_0$};
			\node[cloud, cloud puffs=15.7, cloud ignores aspect, align=center, draw] (cloud) [right of=q0] {$s_2$};
			\path
				(q0) edge node {$\Sigma$} (cloud);
		\end{tikzpicture}
	\end{minipage}
...
\begin{figure}[h!]
	\begin{minipage}{0.25\textwidth}
		\centering
    \[
      s = (i \, \circ_{then} \, a_1) \, \circ_{else} \, a_2
    \]
	\end{minipage}
  ~
	\begin{minipage}{0.25\textwidth}
  ...
  ~
	\begin{minipage}{0.4\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
			...
			\path
        (q0) edge node {$E \wedge E_1$} (q1)
        (q0) edge node {$\bar{E} \wedge E_2$} (q2)
        (q0) edge node [left] {$(E \wedge E_1) \vee (\bar{E} \wedge E_2)$} (q3);
		\end{tikzpicture}
	\end{minipage}
  \caption{A composition of an \textit{if} specification and
    \textit{assert} specifications, with the \textit{if} specification on the
    left-hand side.}
	\label{figure-if-composition-assert}
\end{figure}
...
\begin{figure}[h!]
	\begin{minipage}{0.25\textwidth}
		\centering
    \[
      s = (i \, \circ_{then} \, n_1) \, \circ_{else} \, n_2
    \]
	\end{minipage}
  ~
	\begin{minipage}{0.25\textwidth}
...
def s(event):
  ...
  ~
	\begin{minipage}{0.4\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
			\node[state,initial] (q0) {$q_0$};
			\node[cloud, cloud puffs=15.7, cloud ignores aspect, align=center, draw] (q1) [right of=q0] {$n_1$};
			\node[cloud, cloud puffs=15.7, cloud ignores aspect, align=center, draw] (q2) [below of=q1] {$n_2$};
			\node[state,accepting] (q3) [below of=q0] {$q_3$};
			\path
        (q0) edge node {$E$} (q1)
        (q0) edge node {$\bar{E}$} (q2)
        (q0) edge node [left] {$\Sigma$} (q3);
		\end{tikzpicture}
	\end{minipage}
  \caption{A composition of an \textit{if} specification and
    \textit{next} specifications, with the \textit{if} specification on the
    left-hand side.}
	\label{figure-if-composition-next}
\end{figure}
...
\begin{figure}[h!]
	\begin{minipage}{0.25\textwidth}
		\centering
    \[
      s = (i \, \circ_{then} \, i_1) \, \circ_{else} \, i_2
    \]
	\end{minipage}
  ~
	\begin{minipage}{0.25\textwidth}
  ...
  ~
	\begin{minipage}{0.4\textwidth}
      ...
      \node[state] (q1) [right of=q0] {$q_1$};
			\node[cloud, cloud puffs=15.7, cloud ignores aspect, align=center, draw] (cloud) [below of=q0] {\textit{then} spec};
      \node[state,accepting] (q2) [right of=cloud] {$q_2$};
			\path
				(q0) edge node {$\Sigma$} (q1)
        (q1) edge node {$E$} (cloud)
        (q1) edge node {$\bar{E}$} (q2);
		\end{tikzpicture}
	\end{minipage}

  \caption{A composition of a \textit{next} specification and an \textit{if}
    specification, with the \textit{next} specification on the left-hand side.}
	\label{figure-if-composition-if}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
