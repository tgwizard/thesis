...
verification and unit testing, will be the main point in
...
sometime, the state $S$ will hold before it.

\[
\boldsymbol{G} \, P \rightarrow (\neg P \, \boldsymbol{U} \, (S \wedge \neg P)
\]

(This can be read as: Globally, if $P$ holds, then, while $P$ didn't hold, $S$
held at some point).
...
possible to rewrite the compiled program to add recording functionality. This
can also be done dynamically during runtme, which is especially simple and
suitable for dynamic languages, such as Python, Ruby or JavaScript.
...
Section~\ref{section-testing}.  Here we will discuss the specifics of unit
testing syntax.
...
\begin{figure}[h!] \label{figure-junit}
...
\end{lstlisting}
\caption{An example of unit testing syntax, written for JUnit.}
\end{figure}

JUnit, suites, test cases, set up / tear down. Fixtures? Mocking? This is
"TDD"-style

\subsubsection{"BDD"-style}
