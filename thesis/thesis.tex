...
The result shows that specifications for runtime can be written in the target
program's programming language, in a manner similar to unit tests, and that it
is possible to still give it a formal foundation.
...
\todo{Better hand-over to next section. What exists now?}
...
of verification and formal methods goes back several decades. Research of
...
report. It explains what is meant by proving the correctness of programs in
Section~\ref{section-formal-methods}. Section~\ref{section-rv} describes
runtime verification and its place in proving correctness. And finally,
Section~\ref{section-testing} discusses testing --- syntax, style and other
concepts. But first we start with an essential concept in verification, that of
a \textit{system model}.

\todo{Better introduction to testing}


\section{A Model of a System} \label{section-system-model}

A system model is a construct that represents and describes a system. It is
...
is slow, hard to automate completely, and therefore requires much manual work.
Wading through large programs thus often becomes impractical.
...
Opposite formal methods on the program-correctness-checking spectrum is
...
RV\@. It is in many cases the sole correctness-checking tool used.
...
techniques discussed in Section~\ref{section-formal-methods}.
...
\todo{Better introduction to testing. Same problem as in Background. Should it
be in both places?}
...
clients, to tests (see e.g.\ Section~\ref{section-unit-testing}), and to formal
specifications, written in formal languages, specifying properties that should
verifiably hold for the program (see e.g.\ Section~\ref{section-ltl}). It is
these last two types of specifications that we are interested in here, and
which play an important role in runtime verification.
...
fulfill certain post-conditions (or promises) if the inputs they are given
fulfill the pre-conditions (or requirements) specified in the contract. Design
...
\subsection{Mocking and Faking} \label{section-mocking}
...
\begin{enumerate}
  ...
  \item How will this be used to verify the system against the specification?
    Online or offline verification? Which techniques should be used to verify
    the monitored system against the specification?
...
\begin{description}
  \item[incompleteness] \todo{}

  \item[formal methods] \todo{}
  ...
  \item[model checking] \todo{}

  \item[runtime verification] Verifying an execution of a program. See e.g.\
    Section~\ref{section-rv}.

  \item[self-healing, self-adapting] \todo{}

  \item[state explosion problem] Qdo. \todo{}

  \item[testing] Explain in text \todo{}

  \item[undecidability] \todo{}

  \item[unit testing] Dividing the program into small units, testing each
    separately. See Section~\ref{section-unit-testing}.
\end{description}
