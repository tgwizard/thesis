...
structure. Many interesting properties can be verified using it that ordinary
tests would have trouble with.
...
omständigt syntax och struktur. Många intressanta egenskaper, som vanliga
tester skulle ha problem med, kan verifieras med denna metod.
...
kusin \todo{Create a clearer bond between the two}.
...
# map of FunctionCalls for functions
...
depending on which specification function fails. This is described on the
website for \textit{pythonrv}.
...
above. It first takes arguments specifying what functions should be monitored
(the monitorees), and then the specification function itself. However, the
\texttt{rv.monitor} decorator does not monitor the decorated function, which
here is the specification function, but instead finds the monitorees and
...
In Python, almost all\footnotemark\ functions have a parent \textit{container},
such as a class, an object, or a module. In
Figure~\ref{figure-function-decorator-usages} the functions \texttt{fact}
and \texttt{fact2} belong to the module \texttt{test} (the module's name is
...
For instance, only ever one wrapper per monitoree is created, independent of
the number of specifications that want to monitor it.
...
and thus causing it to propagate up through the call stack. If the exception
...
\textit{pythonrv} also allows for custom error handlers, which can deal with
the specification violations as they see fit. See the website for how.
...
the running program. This is suggested as an area for future work in
