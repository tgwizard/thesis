...
\textit{italics}. These words are, if not explained in the text, have a brief
description in Appendix~\ref{appendix-dictionary}.
...
\begin{itemize}
  ...
  \item Every formal specification function $s$ can be represented by a
    ...
    is a \textit{label}. An automata has one initial state $q_0$, where the
    verification starts, a set of \textit{success states} $S$ and a set of
    \textit{fail states} $F$.

  \item A state cannot be in both $S$ and $F$: $S \cap F = \emptyset$. Both
    fails and success states are marked as \textit{accepting states} in the
    automata below. Fail states are usually called \textit{fail}, $f$ etc.
  ...
  \item The set of \textit{initial transitions} $(q_0, q', E) \in T$, $T
...
trick is to preserve this semantics when transforming and the specifications
into automata, while at the same time allowing for mathematical reasoning to
deduce the correctness of the specifications. This requires two parts. First, a
...
$S = (\alpha_0, \alpha_1, \dots, \alpha_n)$, each of which is a two-part
structure: The function called $\lambda$, and the associated arguments and
state, $\delta$. $\alpha_i = (\lambda_i, \delta_i)$. Since the system model is
an abstraction of an execution of the system (the target program), it is
necessarily finite, but of arbitrary and possibly not-yet complete length.

The system model can be viewed as a directed graph (actually, a tree), where
all nodes but the last have an edge. The first edge is labeled $\alpha_0$, the
second is labeled $\alpha_1$, etc. See Figure~\ref{figure-deduction-example}
(a).
...
specification. An example can be seen in Figure~\ref{figure-deduction-example}
(c).
...
First, the two initial states of $M$ and $C$ are merged, resulting in the state
$(q_0, s_0)$. All transitions $T$ from $q_0$ are merged with the (single)
transition $(s_0, s_1, \alpha_0)$ from $s_0$. This results in a new set of
transitions from $(q_0, s_0)$:
...
This results in a set of second-level states, $Q_2 = \{(q, s_1) | (x, (q, s_1), E)
\in T'\}$, and the process continues. The transitions from all specification
states $q$ from $Q_2$ are merged with the transition $(s_1, s_2, \alpha_1)$
from $s_1$, generating a set of third-level states, $Q_3$. This terminates when
the depth of either the specification or the system model is reached.
...
\begin{figure}[h!]
  ...
  \bigskip
	...
	\begin{minipage}{0.90\textwidth}
      ...
      \node[state,accepting] (f) [below left of=q0] {$f$};
    ...
    \subcaption{The specification automata $C$. This automata is designed to
      ...
      $b$. After that, any events are accepted. $f$ is a fail state and $q_2$
      is a success state.}
	\end{minipage}
  ...
  \bigskip
	...
	\begin{minipage}{0.90\textwidth}
      ...
      \node[state,accepting] (f1) [below left of=q0] {$f_1,s_1$};
      \node[state,accepting] (q2) [below right of=q1] {$q_2,s_2$};
      \node[state,accepting] (f2) [below left of=q1] {$f_2,s_2$};
			\path
...
whether that specification would accept, reject or equivocate the system model.
Start at the root node of $Z$ and follow all edges which labels evaluate to
true. If a fail state is reached, the system model violates the specification.
If all traversals reaches success states, the system model satisfies the
specification. If some traversal ends in a state that is neither a fail or a
success state, the system model can, if it were extended, either satisfy or
violate the specification. \todo{Is this possible? Doesn't the construction
below guarantee no such cases?}
...
specification function. A sketch of the translation procedure, shown in
Figure~\ref{figure-semantics-translation}, is quite simple. The resulting
specification expression has the same semantics as the Python expression. In
the same way that the Python specification operates on an event, a
specification expression is evaluated to true ($\top$) or false ($\bot$) using
the values of an event $\alpha$.
...
\begin{figure}[h!]
    ...
    \begin{tabular}{r c l}
      ...
      $PE(\textit{Accesing a Python function or property})$ & = & \textit{The return value} \\
    \end{tabular}
  \end{minipage}
  \caption{A sketch of the translation procedure of Python expressions to
    specification expressions. $PE$ is the mapping function.}
...
\begin{figure}[h!]
	\begin{minipage}{0.45\textwidth}
      ...
      \node[state,accepting] (fail) [below right of=q0] {\textit{fail}};
			\path
				(q0) edge node {$E$} (q1)
        (q0) edge node {$\bar{E}$} (fail);
		\end{tikzpicture}
    \subcaption{The \textit{assert} specification. $q_1$ is a success state;
      \textit{fail} is a fail state.}
	\end{minipage}
  ~
	\begin{minipage}{0.45\textwidth}
		\centering
...
\subsection{Transforming specifications to automata and rules for composition}
...
$f$ is a fail state, from which no transitions lead. If $\bar{E}$ is true, the
fail state will be reached, and the specification has been violated.
...
\[
  \begin{array}{rcl}
    A(s) & = & (Q_s, T_s \cup P_s, q_{0s}, S_s, F_s) \\
   A(s') & = & (Q_{s'}, T_{s'} \cup P_{s'}, q_{0s'}, S_{s'}, F_{s'})
  \end{array}
\]
\medskip
...
\[
  \begin{array}{rcl}
    A(s \circ_{tail} s') & = & (\{q_0\} \cup Q_s \cup Q_{s'} - \{q_{0s}, q_{0s'}\}, T \cup (P_s \cup P_{s'}), q_0, S_s \cup S_{s'}, F_s \cup F_{s'}) \\
  ...
  ~
	\begin{minipage}{0.45\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
			...
			\node[state,accepting] (f) [right of=qy] {$f$};
			\node[state,accepting] (qw) [below left of=q0] {$q_w$};
			\node[state,accepting] (f') [left of=qw] {$f'$};
			\path
...
def s'(event):
  ...
  \caption{A composition of a \textit{next} specification and an
    \textit{assert} specification, using the \textit{next} composition point.}
