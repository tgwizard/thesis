...
simpler. \todo{Move to where they are first used}
...
\begin{mydef}\label{def-composition-complete}
...
\begin{mydef}\label{def-automata}
...
\begin{mydef}\label{def-accepting-and-fail-states}
A state cannot be both accepting and failing --- it cannot be in both $S$ and
$F$: $S \cap F = \emptyset$. Success states are depicted as \textit{accepting
states} in the automata. Fail states are usually called \textit{fail}, $f$,
etc.
\end{mydef}
...
initial transitions $T$, and all other transitions $R$: $P = T \cup R$.
\end{mydef}
...
--- even a list), where all nodes but the last have an edge. The first edge is
labeled $\alpha_0$, the second is labeled $\alpha_1$, etc. See
...
\subsubsection{Composition of the \textit{if-then} and \textit{if-then-else}
specifications}
...
the $\neg E$ guard goes to an accepting state.
...
And for \textit{if-then-else} specifications compositions becomes:
