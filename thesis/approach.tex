...
specifications. The automata $\llbracket s_a(E) \rrbracket$ for a standalone
\texttt{assert} specification $s_a(E)$, depicted in
Figure~\ref{figure-basic-formal-specification-automata}~(a), is defined as:

\medskip
\[
  \begin{array}{rcl}
    \llbracket s_a(E) \rrbracket & = & (\{q_0, q_1, f\}, \{(q_0, q_1, E), (q_0, f, \neg E)\}, q_0, \{q_1\}, \{f\})
  \end{array}
\]
\medskip
...
\[
  \begin{array}{rcl}
    \llbracket s \circ_{tail} s' \rrbracket & = & (Q, P, q_0, S_s \cup S_{s'}, F_s \cup F_{s'}) \\
...
\begin{figure}[h!]
    ...
    \[
      \begin{array}{rcl}
        \llbracket s_a(E) \rrbracket & = & (\{q_x, q_y, f\}, \{(q_x, q_y, E), (q_x, f, \neg E)\}, q_x, \{q_y\}, \{f\}) \\
       \llbracket s_a(E') \rrbracket & = & (\{q_z, q_w, f'\}, \{(q_z, q_w, E') ,(q_z, f', \neg E')\}, q_z, \{q_w\}, \{f'\})
      \end{array}
    \]
		\centering
    \[
      \begin{array}{rcl}
        \llbracket s_a(E) \circ_{tail} s_a(E') \rrbracket & = & (Q, P, q_0, S, F) \\
                                 Q & = & \{q_0, q_y, f, q_w, f'\} \\
...
def s():
  ...
  \caption{An example showing a composition of an \texttt{assert} specification
    ...
    resulting $\llbracket s_a(E) \, \circ_{tail} \, s_a(E) \rrbracket$ automata
    is shown. $q_y$ and $q_w$ are success states; $f$ and $f'$ are fail
    states.}
...
with $\llbracket s \rrbracket = (Q, P, q_0, S, F)$ is as follows:

\medskip
\[
  \begin{array}{rcl}
    \llbracket s_n \circ_{next} s \rrbracket & = & (\{q_0'\} \cup Q, T \cup P, q_0', S, F) \\
               T  & = & \{(q_0', q_0, \top)\}
  \end{array}
\]
\medskip
...
Suppose two arbitrary specifications $s_1$ and $s_2$ with semantics defined as:
...
\[
  \begin{array}{rcl}
  \llbracket s_i(E) \circ_{then} s_1 \rrbracket & = & (Q, P, q_0, S, F) \\
...
\[
  \begin{array}{rcl}
  \llbracket (s_i(E) \circ_{then} s_1) \circ_{else} s_2 \rrbracket & = & (Q, P, q_0, S, F) \\
