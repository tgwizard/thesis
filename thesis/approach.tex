...
\footnotetext[4]{This is not true for closures --- functions defined inside
  ...
  from outside of the defining function. As of writing, \textit{pythonrv} does
  not support monitoring of closures.}
...
$s \; ; \; s'$ denotes a \textit{conjunctive composition} in that both
...
\begin{figure}[h!]
  ...
  ~
	\begin{minipage}{0.45\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
			\node[state,initial] (q0)	{$q_0$};
			\node[cloud, cloud puffs=15.7, cloud ignores aspect, align=center, draw] (cloud) [right of=q0] {$\llbracket next \rrbracket$};
			\path
  ...
  \bigskip
	...
	\begin{minipage}{0.45\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
			\node[state,initial] (q0)	{$q_0$};
			\node[cloud, cloud puffs=15.7, cloud ignores aspect, align=center, draw] (then) [right of=q0] {$\llbracket then \rrbracket$};
  ...
  ~
	\begin{minipage}{0.45\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
			\node[state,initial] (q0)	{$q_0$};
			\node[cloud, cloud puffs=15.7, cloud ignores aspect, align=center, draw] (then) [right of=q0] {$\llbracket then \rrbracket$};
			\node[cloud, cloud puffs=15.7, cloud ignores aspect, align=center, draw] (else) [below of=then] {$\llbracket else \rrbracket$};
			\path
				(q0) edge node {$E$} (then)
        (q0) edge node {$\neg E$} (else);
		\end{tikzpicture}
...
%%%%%%%%%%%%%%%%%%%%%%%%
%% Composition, next s, symbol-python-automata
%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h!]
	\begin{minipage}{0.25\textwidth}
		\centering
    \[
      \llbracket s' \rrbracket = \llbracket \texttt{next} \; s \rrbracket
    \]
	\end{minipage}
  ~
...
def s'():
  ...
  ~
	\begin{minipage}{0.35\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
			\node[state,initial] (q0')	{$q_0'$};
			\node[cloud, cloud puffs=15.7, cloud ignores aspect, align=center, draw] (cloud) [right of=q0'] {$\llbracket s \rrbracket$};
			\path
				(q0') edge node {$\top$} (cloud);
		\end{tikzpicture}
	\end{minipage}
  \caption{The semantics for a $\texttt{next} \; s$ specification, given an
    arbitrary specification $s$.}
...
\texttt{if-then} specifications are of the form $\texttt{if} \; E \;
\texttt{then} \; s_1$, and \texttt{if-then-else} specifications of the form
$\texttt{if} \; E \; \texttt{then} \; s_1 \; \texttt{else} \; s_2$.

In an \texttt{if-then-else} specification we consider the expression $E$ as a
guard for the specification $s_1$ in the \textit{then} clause, and $\neg E$ as a
guard for the specification $s_2$ in the \textit{else} clause. In
\texttt{if-then} specifications, the $\neg E$ guard goes to a success state.

To make the definitions below simpler we introduce the concept of
...
Furthermore, we define the semantics of $\texttt{if} \; E \; \texttt{then} \;
...
An example for composing two \texttt{assert} specifications this way is shown
in Figure~\ref{figure-assert-composition-example}. The resulting automata is
...
%%%%%%%%%%%%%%%%%%%%%%%%
%% Composition, assert E ; assert E', symbol-python-automata
%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[h!]
    ...
    \[
      \begin{array}{rcl}
        \llbracket \texttt{assert} \; E \rrbracket & = & (\{q_x, q_y, f\}, \{(q_x, q_y, E), (q_x, f, \neg E)\}, q_x, \{q_y\}, \{f\}) \\
       \llbracket \texttt{assert} \; E' \rrbracket & = & (\{q_z, q_w, f'\}, \{(q_z, q_w, E') ,(q_z, f', \neg E')\}, q_z, \{q_w\}, \{f'\})
      \end{array}
    \]
		\centering
    \[
      \begin{array}{rcl}
        \llbracket \texttt{assert} \; E \; ; \; \texttt{assert} \; E' \rrbracket & = & (Q, P, q_0, S, F) \\
                 Q & = & \{q_0, q_y, f, q_w, f'\} \\
  ...
  ~
	\begin{minipage}{0.45\textwidth}
      ...
      grid,auto,scale=.5,semithick,>=stealth]
		...
		\end{tikzpicture}
	\end{minipage}
  \caption{An example showing $\texttt{assert} \; E \; ; \; \texttt{assert} \;
    E'$ automata is shown. $q_y$ and $q_w$ are success states; $f$ and $f'$ are
    fail states.}
...
First we recall some theory about automata and define some simple properties about them,
...
Given an event $\alpha$ and an expression $E$ we say that $\alpha \models E$
  ...
  \bigskip
	...
	\begin{minipage}{0.90\textwidth}
      ...
      \node[state] (f2) [below left of=q1] {$f_2,s_2$};
			\path
        (q0) edge node {$((a, \epsilon), \lambda = a)$} (q1)
        (q0) edge node [above left] {$((a, \epsilon), \lambda \neq a)$} (f1)
        (q1) edge node {$((b, \epsilon), \lambda = b)$} (q2)
        (q1) edge node [above left] {$((b, \epsilon), \lambda \neq b)$} (f2);
		\end{tikzpicture}
...
\begin{figure}[h!]
  ...
  \bigskip
    ...
    \[
      \begin{array}{rcl}
        \llbracket \text{spec} \rrbracket & = & (Q, P, q_0, S, F) \\
                     Q & = & \{q_0, q_1, f\} \\
  ...
  \bigskip
	...
	\begin{minipage}{0.95\textwidth}
      ...
      \node[state] (f1') [left of=q0'2] {$(f,s_1)'$};
			\path
        (q0) edge node {$((\lambda_0, \delta_0), \delta(\texttt{func.inputs$[0]$}) > 0)$} (q1)
        (q0) edge node [above left] {$((\lambda_0, \delta_0), \neg (\delta(\texttt{func.inputs$[0]$}) > 0))$} (f1)
        (q0) edge node [right] {$\top$} (q0')
        (q0') edge node {$((\lambda_1, \delta_1), \delta(\texttt{func.inputs$[0]$}) > 0)$} (q1')
        (q0') edge node [above left] {$((\lambda_1, \delta_1), \neg (\delta(\texttt{func.inputs$[0]$}) > 0))$} (f1')
